...
| `hide-delegate`        | design     | advisory          | >= 3 dot-accesses      | A method/field-access chain in one expression (`a.getB().getC().doThing()`) reaches this many `.member` hops or more — a Law of Demeter violation; Fowler's *Hide Delegate*. Excludes a chain where any hop's call passes at least one argument (the fluent-builder/stream-pipeline proxy: `.filter(pred).map(f)` almost always threads an argument through a hop, while a real accessor-chain violation is near-universally zero-arg `getX()`/`getY()`). Excludes a chain rooted at the enclosing method's own receiver (`self.a.b.c()`/`this.a.b.c()`, reaching through your own encapsulated state isn't what Demeter targets) or at what looks like a type/namespace reference (`MonotonicClock.Global.approxTime.now()` names a single static value, not an object graph). Go has no fixed receiver keyword (the name is arbitrary per method), so the receiver exclusion doesn't apply there. Real-world backtest (`docs/backtest-repos.md`): both exclusions together cut `BurntSushi/ripgrep`'s raw finding count 212 → 37 (a spot check of the remainder found genuine multi-hop collaborator reaches, e.g. `chir.hir().properties().look_set().contains_word_unicode()`). `apache/cassandra`'s much higher remaining count (2514) reflects that codebase's getter-heavy enterprise style rather than checker noise — a 6-sample spot check there (`metrics/TableMetrics.java:528`, `tools/NodeTool.java:191`, `net/OutboundConnection.java:321`, others) found 6/6 genuine collaborator-reaching chains, 0 false positives. `kubernetes/kubernetes`'s Go corpus (no receiver-keyword exclusion available) showed markedly more noise; accepted as a known, documented gap rather than guessed at (see `src/checkers/rules.rs`'s `CHAIN_SELF_LIKE_BASE_NAMES` doc comment). |
