...
|-----------------|------------------|
| `[verbose-comment]` | A comment contains marketing/hedge-word filler (`seamlessly`, `powerful`, `robust`, `enterprise-grade`, `leverage`, `utilize`, `it's worth noting`, `as mentioned above`, `note that`, `needless to say`) or invented rationale (`designed to improve`, `supports future`), or references the change instead of the code (`used by`, `added for the`, `this fix`, `this PR`, `handles the case from issue`) — that belongs in the commit message, not a comment that outlives it. Matched case-insensitively, substring. |
