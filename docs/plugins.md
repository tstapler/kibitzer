...
without hand-editing every repo's `.kibitzer/inspect.json` to add a `command`
...
[kibitzer] kibitzer-stub-plugin now runs as a check in every repo on this machine.
  - disable it for one repo: add "disabled": ["kibitzer-stub-plugin"] to that repo's .kibitzer/inspect.json
...
`.kibitzer/inspect.json` still references the plugin by a hand-authored check
entry.
...
`.claude/`, not `.kibitzer/inspect.json` or anything under
...
that repo's `.kibitzer/inspect.json` (see `docs/suppressing-checks.md`); to
...
way its `config_stamp` fingerprints `.kibitzer/inspect.json`, so a changed
