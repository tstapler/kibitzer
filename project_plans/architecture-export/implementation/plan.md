...
- `--dry-run` prints the would-be file contents to stdout and does not write `arch.json`, matching `install.rs`'s `--dry-run` behavior exactly (same flag name, same "would write" framing).
  - *Given* the same fixture, *When* run with `--dry-run`, *Then* stdout contains the JSON and no `arch.json` file is created on disk.
