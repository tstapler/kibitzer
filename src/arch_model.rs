...
/// Whether a [`FieldAccessEdge`] reads or writes the field. v1 write-detection is
/// syntactic only (see `FieldAccessEdge`'s doc comment) — a case this doesn't recognize as
/// a write (e.g. `&recv.Field` handed to something that mutates it indirectly) is counted
/// as `Read`. Known ceiling, not fixed for v1.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum AccessKind {
    Read,
...
/// One field access: a method reading or writing a struct field on its own receiver
/// (`recv.Field`), found by matching a `selector_expression`'s operand against the
/// enclosing method's receiver identifier. Go-only for v1 — the same "reuse existing
/// per-language coverage" scoping `RawCallSite`/`call_graph_supports` already applies to
/// the call graph.
///
/// `to` is a synthetic field id (`"{package}::{type}.{field}"`, matching `SymbolNode::id`'s
/// owner-qualified scheme) rather than a `SymbolNode::id` — fields aren't `SymbolNode`s
/// (`SymbolKind` has no `Field` variant). Unlike `CallEdge`, there is no "unresolved"
/// variant: a site whose accessed name doesn't match any field declared on the receiver's
/// type (e.g. a value passed to a call on that selector, or a typo the compiler would have
/// caught) is simply dropped during resolution rather than kept as a guess.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct FieldAccessEdge {
    pub from: String,
    pub to: String,
...
/// query, LSP symbols, diagram) read from.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// A file whose contents (case-insensitive) mention "do not edit" or "code generated" is
/// treated as machine-generated and skipped entirely by `build_model` — matching common
/// generator banners (e.g. `// Code generated by protoc-gen-go. DO NOT EDIT.`).
///
/// Scans the whole file, not a fixed line window: a real-world generated file's banner
/// position varies with its license-header length, which this codebase can't predict. A
/// backtest across every `zz_generated*.go` in `kubernetes/kubernetes` found the banner
/// anywhere from line 17 to line 29 depending on header shape (a `//go:build` tag line
/// present or not, license-block length) — a fixed window sized to that sample would still
/// be one header line away from silently missing a file in some other codebase's
/// convention, the same way this function's original 2-line window did. The scan itself is
/// cheap relative to what it's meant to avoid: `build_model` already holds the whole file
/// in memory here, and skipping tree-sitter parsing entirely on a match more than pays for
/// a linear pass over the (often huge) generated file's text.
...
/// `file`'s field-access sites with `file` filled in — the `FieldAccessEdge` sibling of
/// `collect_call_sites`. Resolution against the whole-package struct-field index still
/// happens later, in `resolve_field_access_edges`.
fn collect_field_access_sites(
    language: Language,
    source: &str,
    tree: &tree_sitter::Tree,
    package_path: &str,
...
pub fn build_model(
    ...
    for (path, source) in files {
        ...
        for (type_name, field_name) in extract_struct_fields_for_file(language, source, &tree) {
            struct_fields
                .entry((package_path.clone(), type_name))
                .or_default()
                .insert(field_name);
        }
        raw_field_access_sites.extend(collect_field_access_sites(
            language,
            source,
            &tree,
            &package_path,
...
/// `(package_path, receiver_type)` is dropped rather than kept as an unresolved edge (see
/// `FieldAccessEdge`'s doc comment for why that differs from `CallEdge`'s convention).
fn resolve_field_access_edges(
    struct_fields: &HashMap<(String, String), std::collections::HashSet<String>>,
    raw_sites: Vec<RawFieldAccessSite>,
) -> Vec<FieldAccessEdge> {
    raw_sites
        .into_iter()
        .filter(|site| {
            struct_fields
                .get(&(site.package_path.clone(), site.receiver_type.clone()))
                .is_some_and(|fields| fields.contains(&site.field_name))
        })
        .map(|site| FieldAccessEdge {
            from: site.caller_id,
            to: format!(
                "{}::{}.{}",
                site.package_path, site.receiver_type, site.field_name
            ),
            access: site.access,
            file: site.file,
            line: site.line,
...
mod tests {
    ...
    #[test]
    fn build_model_resolves_field_access_when_struct_and_method_are_in_different_files() {
        // The struct declaration and the method reading its field live in separate files
        // of the same package — resolution must work off the whole-package field index
        // built while walking every file, not just the file the method itself is in.
        let repo_root = PathBuf::from("/repo");
        let files = vec![
            (
                PathBuf::from("/repo/pkg/types.go"),
                "package pkg\n\ntype T struct {\n\tX int\n}\n".to_string(),
            ),
            (
                PathBuf::from("/repo/pkg/methods.go"),
                "package pkg\n\nfunc (t T) Get() int {\n\treturn t.X\n}\n".to_string(),
            ),
        ];
        let model = build_model(
        ...
        assert_eq!(
            model.field_accesses.len(),
            1,
            "got: {:?}",
            model.field_accesses
        );
        let edge = &model.field_accesses[0];
        assert_eq!(edge.from, "pkg::T.Get");
        assert_eq!(edge.to, "pkg::T.X");
        assert_eq!(edge.access, AccessKind::Read);
    }

    #[test]
    fn build_model_drops_a_field_access_site_whose_name_matches_no_declared_field() {
        // `t.NotAField` can't correspond to a real access site in valid Go (the compiler
        // would reject it), but this proves resolution doesn't invent an edge for a
        // selector name it can't confirm against the struct's own field list — see
        // `FieldAccessEdge`'s doc comment on why an unmatched site is dropped, not kept as
        // an "unresolved" edge the way `CallEdge` would.
        let repo_root = PathBuf::from("/repo");
        let files = vec![(
            PathBuf::from("/repo/pkg/a.go"),
            "package pkg\n\ntype T struct {\n\tX int\n}\n\nfunc (t T) Get() int {\n\treturn t.NotAField\n}\n"
    ...
    #[test]
    fn looks_generated_finds_the_banner_behind_a_license_header() {
        // Regression for a real false positive found backtesting the LCOM checker against
        // k8s.io/apimachinery: `zz_generated.deepcopy.go`'s "Code generated" banner sits at
        // line 20, behind an Apache license block and `//go:build` tags — the old 2-line
        // scan window missed it entirely.
        let source = "//go:build !ignore_autogenerated\n\
             /*\n\
    ...
    /// Regression for a second real defect: an earlier fix widened the scan to a fixed
    /// 20-line window, but real generator headers in the very same repo ranged from line
    /// 17 to line 29 depending on header shape — a fixed window is always one header line
    /// away from missing some other file. This test's banner sits at line 59, well past
    /// any fixed window this function has ever used, to actually prove there's no
    /// remaining line limit rather than coincidentally fitting inside the last one chosen.
    #[test]
    fn looks_generated_finds_a_banner_far_past_any_previous_fixed_window() {
        let filler = "// filler comment line\n".repeat(58);
        let source =
            format!("{filler}// Code generated by some-generator. DO NOT EDIT.\n\npackage v1\n");
        assert_eq!(
            source.lines().count() - 2,
            59,
            "sanity: banner is on line 59"
        );
        assert!(looks_generated(&source));
    }

    #[test]
    fn looks_generated_is_false_for_a_normal_file_mentioning_neither_phrase() {
        assert!(!looks_generated("package pkg\n\nfunc A() {}\n"));
    }
