...
/// position varies with its license-header length, which this codebase can't predict, so
/// any fixed window risks silently missing a file in some other codebase's convention. The
/// scan is cheap relative to what it avoids — `build_model` already holds the whole file in
/// memory, and skipping tree-sitter parsing on a match more than pays for the linear pass.
