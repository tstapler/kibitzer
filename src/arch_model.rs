...
/// A file whose first 20 lines (case-insensitive) mention "do not edit" or "code
...
/// EDIT.`). 20 lines, not 2: a real-world generated file often has a license header (and,
/// for Go, `//go:build` tags) *before* the codegen banner — confirmed by an LCOM backtest
/// against `k8s.io/apimachinery`, where `zz_generated.deepcopy.go`'s banner sits at line 20
/// behind an Apache license block, so the old 2-line window silently let its methods
/// through and skewed cohesion metrics with generated boilerplate.
